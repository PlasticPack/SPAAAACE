...
std::vector<std::vector<int>> circularMask(const std::vector<std::vector<int>> &rought, const int &radius){
	...
	for (int x = 0; x < min; x++){
		for (int y = 0; y < min; y++){
			if (min % 2 == 1){
				if (sqrt(pow(min/2+1 - x, 2) + pow(min/2+1 - y, 2)) <= radius){
					pure[x][y] = rought[x][y];
				}
				else pure[x][y] = 0;
			}
			else{
				if ((sqrt(pow(min / 2 + 1 - x, 2) + pow(min / 2 + 1 - y, 2)) <= radius) || sqrt(pow(min / 2 - x, 2) + pow(min / 2 - y, 2)) <= radius){
					pure[x][y] = rought[x][y];
				}
				else pure[x][y] = 0;
			}
...
std::vector<genesis::star> genesis::generateSystem(const std::string& filepath,const std::string& seed){
	...
	for (int star = 0; star < count_stars; star++){
		...
		for (int planet = 0; planet < count_planet; planet++){
			...
			if (radius < radius_i){
				type = hash_int(hash_str("You'reTheHotType!" + std::to_string(rand())) % seed_hashed) % (TYPE_DESERT_MARS + 1);
				
			}
			else if (radius < radius_s){
				type = hash_int(hash_str("Earthlike!" + std::to_string(rand()*radius)) % seed_hashed) % (TYPE_EARTHLIKE_MAGMA - TYPE_DESERT_MARS + 1) + TYPE_DESERT_MARS+1;
			}
			else{
				if (current_planet.mass > 80){
					type = hash_int(hash_str("ColdType_notMyTypeMhe..." + std::to_string(rand())) % seed_hashed) % (TYPE_GAS_WHITESHELTER - TYPE_EARTHLIKE_MAGMA + 1) + TYPE_EARTHLIKE_MAGMA+1;
				}
				else{
...
#define phys(o) o.get<PhysicsComponent>()
		...
		int i = 0;
		for (genesis::star current_star : genesis::generateSystem(filepath, seed)){
			std::cout << "*" << std::endl;
...
//#define PC2PX(p) Vec2(p.x()*(2/3)*pow(10,12), p.y()*(2/3)*pow(10,12))
				...
				obj_star.get<PositionComponent>()->setPosition(Vec2(current_star.position.x()*2/3*pow(10,12),current_star.position.y()*2/3*pow(10,12)));
				std::cout << phys(obj_star)->getPosition().x()<<std::endl;
				std::cout << phys(obj_star)->getPosition().y() << std::endl;
				std::cout << current_star.position.x()<<std::endl;
				std::cout << current_star.position.y() << std::endl;
				phys(obj_star)->setMass(current_star.mass * 333000);
				pure_objs.push_back(std::make_shared<GameObject>(obj_star));
				i++;
				int j = 0;
				for (genesis::planet current_planet : current_star.planets){
					...
					if (obj_planet.hasComponent(typeid(PhysicsComponent)) && obj_planet.hasComponent(typeid(PositionComponent))){
						...
						phys(obj_planet)->setMass(current_planet.mass);
						//obj_planet.get<PositionComponent>()->getPosition.
						//phys(obj_planet)->setPosition((current_planet.position));
						obj_planet.get<PositionComponent>()->setPosition((Vec2	(current_planet.position.x()*2/3*pow(10,12),current_planet.position.y()*2/3*pow(10,12))));
						...
						if (AIs.end() != AIs.begin()){
							...
							if ((current_planet.type > TYPE_DESERT_MARS) && (current_planet.type < TYPE_ICE_ICE_BABY)){
								...
								for (int nem = 0; nem < nem_count; nem++){
									...
									currentAI.get<PositionComponent>()->setPosition(Vec2(cos(teta)*dis + obj_planet.get<PositionComponent>()->getPosition().x(), sin(teta)*dis + obj_planet.get<PositionComponent>()->getPosition().y()));
...
//#undef PC2PX
		...
		if (rought_objs.find("objective_1") != rought_objs.end() && rought_objs.find("objective_2") != rought_objs.end() & rought_objs.find("objective_3") != rought_objs.end()){
			std::vector<GameObject> objectives = { *rought_objs.find("objective_1")->second, *rought_objs.find("objective_2")->second, *rought_objs.find("objective_3")->second };

			for (int objective = 0; objective<3; objective++){
				srand(objective);

				int pos = hash_int(hash_str("Objective" + std::to_string(rand())) % (seed_hashed + 1)) % planets.size();
				int dis = hash_int(hash_str("Dis" + std::to_string(rand())) % (seed_hashed + 1)) % 500;
				double teta = hash_int(hash_str("Teta" + std::to_string(rand())) % (seed_hashed + 1)) % 36000; teta /= 100;
				objectives[objective].get<PositionComponent>()->setPosition(Vec2(planets[pos]->get<PositionComponent>()->getPosition().x() + dis*cos(teta), planets[pos]->get<PositionComponent>()->getPosition().y() + dis*sin(teta)));
				objectives[objective].get<PhysicsComponent>()->setHitboxRadius(250);
				//removes the objective's parent from the stack so that objectives are unique
				planets.erase(planets.begin() + pos);
				pure_objs.push_back(std::make_shared<GameObject>(objectives[objective]));
			}
		}
	}
	...
	std::string name[] = { "hud_base_pointer", "hud_fuel", "hud_fuel_container", "hud_life","hud_life_container", "hud_obj_pointer","base" };
	for (std::string hc : name){
...
int main(int argc, char* argv[]){
	...
	for (int type = 0; type < TYPE_GAS_WHITESHELTER; type++)
	{
